...
eq_quantity <- agg_func(el_price_cents)
...
data_check <- data %>% 
  mutate(high_est = high_func(price_cents),
         ...
         supply = supply_func(x1))

ggplot(data = data_check)+
  geom_line(aes(x = q_high_kwh, y = price_cents), color = 'black')+
  geom_line(aes(x = high_est, y = price_cents), color = 'blue')+
  geom_line(aes(x = q_low_kwh, y = price_cents), color = 'green')+
  geom_line(aes(x = low_est, y = price_cents), color = 'red')+
  geom_line(aes(x = q_low_kwh+q_high_kwh, y = price_cents), color = 'cyan')+
  geom_line(aes(x = agg_est, y = price_cents), color = 'orange')+
  geom_line(aes(x = supply, y = x1), color = 'orange')+
  geom_point(aes(x = eq_quantity, y = el_price_cents))

```

3. Aggregate monthly demand curve is horizontal sum of high and low income monthly demand curves. Start by creating linear fit of quantity demanded as function of price for both income groups. Given these are in the same format coefficients can be directly summed.
...
data <- data %>% 
  mutate(high_est = high_func(price_cents),
         ...
         mec = mec_func(price_cents))

ggplot(data = data_check)+
  geom_line(aes(x = high_est, y = price_cents), color = 'blue')+
  geom_line(aes(x = low_est, y = price_cents), color = 'red')+
  geom_line(aes(x = agg_est, y = price_cents), color = 'green')+
  geom_line(aes(x = supply, y = price_cents), color = 'orange')+
  geom_line(aes(x = mec, y = price_cents), color = 'cyan')+
  geom_point(aes(x = eq_quantity, y = el_price_cents))

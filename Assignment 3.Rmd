...
quantity = (low_fit$coefficients[2]+ high_fit$coefficients[2])*price + (low_fit$coefficients[1] + high_fit$coefficients[1])
}

agg_func_inv <- function(quantity){
price = (quantity - (low_fit$coefficients[1] + high_fit$coefficients[1])) /(low_fit$coefficients[2]+ high_fit$coefficients[2])
}
...
low_int_quantity <- round(uniroot(function(quantity) supply_func_inv(quantity)-low_func(quantity), c(0,400000), tol = .01)$root,0)
...
high_exc <- integrate(high_func_inv, 0, high_int_quantity)$value  - integrate(supply_func_inv, 0, eq_quantity)$value

low_exc <- integrate(low_func_inv, 0, low_int_quantity)$value  - integrate(supply_func_inv, 0, eq_quantity)$value
...
ggplot(data = data_check)+
  ...
  geom_point(aes(x = high_int_quantity, y = low_func_inv(high_int_quantity)))+
  geom_point(aes(x = low_int_quantity, y = high_func_inv(low_int_quantity)))
